 ...
 # lpt.py
 #
 # Map the tasks of the graph using the LPT heuristic presented in
 # the following paper:
			...
			if thr_num == 0 and func.check_empty_thr(num_threads, thread_queue) == True:
				if bool(task_stack):
					...
					for index in range(len(sort_idle_list)):
						...
						# Choose one of the tasks from the selected tasks by the LPT heuristic #
...
# The main function #
def execute(num_tasks, num_tasksets, num_threads, task_list, period, deadline, graphic_result, dot_file_path, yaml_file_path):
	global task_stack, last_idle, thread_queue, comp_tasks_cnt
	...
	func.export_scheduling(num_threads, thread_queue, 'lpt', '', '')

	# Write the information to the YAML file #
	func.write_yaml(period, deadline, num_tasks, task_list, num_tasksets, num_threads, thread_queue, dot_file_path, yaml_file_path)

	# Draw the graphical output #
	if graphic_result == 1:
		func.graphic_result(num_threads, thread_queue, t, 'lpt', '', '')

	# Return the results to the main program #
	#return response_time, idle_time, miss_deadline
	return response_time, idle_time
