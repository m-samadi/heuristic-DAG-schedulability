...
int main(int argc, char **argv){
    ...
    //Worst-fit processors assignment
    bool wf_proc_assign = false;
    if(argc > 3)
        wf_proc_assign = atoi(argv[3]);  

    //Scheduling type (partitioned, global)
    int sched_type = -1;
    if(argc > 4)
        sched_type = atoi(argv[4]);  
    ...
    //Create and open a text file to write the results of the schedulability analysis to the file
    std::ofstream out_file("sched_result.txt");
    ...
    if(constrained_taskset){
        ...
        if(sched_type == 1){
            ...
            out_file<< "global: "   <<dagSched::GP_LP_FTP_Serrano16_C(taskset, n_proc)<<std::endl;
        }        
        ...
        if(sched_type == 0){
            ...
            out_file<< "partitioned: "<<dagSched::P_LP_FTP_Casini2018_C(taskset, n_proc)<<std::endl;
        }
        #ifdef ZAHAF2019
