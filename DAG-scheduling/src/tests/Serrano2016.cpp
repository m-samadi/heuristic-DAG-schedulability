...
bool GP_LP_FTP_Serrano16_C(Taskset taskset, const int m){
    ...
    while(at_least_one_update){
        ...
        for(int i=0; i<taskset.tasks.size(); ++i){
            ...
            std::ofstream out_file1("global_result.txt");
            out_file1<<"response time: "<<R_old[i]<<std::endl;
            out_file1.close();
            ...
            //Create and open a text file to write the response time to the file
            std::ofstream out_file2("global_result.txt");
            out_file2<<"response time: "<<R[i]<<std::endl;
            out_file2.close();
