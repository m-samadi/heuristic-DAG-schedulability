...
float computeWCRTDAG(const Taskset& taskset, const int task_idx, const bool joint){
    ...
    for(const auto& p:all_paths){
        ...
        pathAnalysis(p, self, taskset, task_idx, RTs, joint, false);
    }
...
bool P_FP_FTP_Fonseca2016_C(Taskset taskset, const int m, const bool joint){
    ...
    for(int i=0; i<taskset.tasks.size(); ++i){
        ...
        for(const auto& p:pVol){
            //Create and open a text file to write the response time to the file
            std::ofstream out_file1("partitioned_result.txt");
            out_file1<<"response time: "<<p.second<<std::endl;
            out_file1.close();
    ...
    if(at_least_one_update){
        ...
        for(int i=0; i<taskset.tasks.size(); ++i){
            ...
            //Create and open a text file to write the response time to the file
            std::ofstream out_file2("partitioned_result.txt");
            out_file2<<"response time: "<<R<<std::endl;
            out_file2.close();
